...
# The image build validates Docker before the image is cloned. Docker records the
# default bridge network in local-kv.db; clearing it lets each clone recreate a
# clean docker0 bridge on first boot.
