...
| --- | --- | --- | --- |
| Docker Sandboxes (primary) | Selected Catthehacker Full/Act source or the matching official EPAR prebuilt package | Verified imported runner template | `image build` or startup prebuilt acquisition |
...
The first-run wizard offers Docker Sandboxes with four choices: verified prebuilt Full as the default, verified prebuilt Act, local Catthehacker Full, and local Catthehacker Act; choose `C. Show compatibility providers` to reach Docker Container and WSL, which retain another validated Catthehacker tag. Tart is retired from onboarding, although existing Tart configurations continue to build and clean up their VM image. Docker Sandboxes local builds offer only `full-latest` and `act-latest` because the reusable template must already contain the private Docker daemon and runtime closure. The local Full choice warns that preparation can take 30–60 minutes. Every local-build path includes platform resolution and a storage estimate; prebuilt acquisition and import sizing are resolved during startup after digest and attestation verification. The generated config uses no custom scripts and schedules updates weekly at 07:00 local time; edit the config after initialization to change those advanced settings. `./start` always verifies local inputs and the active artifact, but checks mutable package aliases, source tags, and `runnerVersion: latest` only when the configured schedule is due. Docker Sandboxes imports a replacement into its template cache and activates it only after exact readback succeeds.
...
For `image.distribution: prebuilt`, EPAR acquires an exact verified EPAR package platform and imports it through the same transactional receipt/readback path. Before a cold acquisition it reports a deliberately broad historical reference: Act commonly writes about 0.8-2 GiB in 2-15 minutes, Full amd64 about 16-24 GiB in 15-60 minutes, and Full arm64 about 8-16 GiB in 15-60 minutes. These ranges are orientation rather than completion thresholds because upstream contents and host conditions change. Interactive startup then redraws one bounded progress line while the archive is downloaded and materialized, reporting bytes written, average archive-write rate, elapsed time, and the later hashing and structural-verification phases; non-interactive operation records lower-frequency progress heartbeats. The archive-write rate is not presented as network throughput, and EPAR does not claim a percentage because the signed OCI catalog does not contain the final uncompressed Docker-archive size. Normal prebuilt use resolves the signed stable catalog; operator-only `image.prebuiltAcceptance: true` instead requires an exact immutable candidate catalog, exact package digest, and evidence ref and never follows the stable alias. The public base is reused without redownload when already cached. Custom scripts create a small local derivative from that base, while runtime trust and network overlays remain outside the public image. See [Docker Sandboxes prebuilt image publication and acceptance](development/docker-sandboxes-prebuilt.md).
