...
The reusable template disables and masks Ubuntu's periodic apt units. Docker Sandboxes also launches one fixed, best-effort `apt-get update -qq -y` during guest boot; before GitHub runner registration EPAR identifies that exact parent command and executable, terminates only that bootstrap refresh, and then waits for every apt/dpkg process to release its locks. An unexpected package operation is never killed by a broad name or pattern and instead fails registration closed after the bounded wait. This keeps warm replacement runners from inheriting `/var/lib/apt/lists/lock`, while ordinary workflow-initiated package commands remain available afterward.
