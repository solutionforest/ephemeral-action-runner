...
EPAR preserves the Docker Sandboxes v0.39 missing-sandbox cleanup signatures and recognizes the standardized v0.42.1 response `sandbox '<name>' not found` only when `<name>` exactly matches the instance being stopped or deleted. v0.42.1 compatibility is not live-validated yet; after upgrading, validate it in a disposable pool and treat any other new provider behavior as unsupported until verified.
