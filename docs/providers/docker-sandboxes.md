...
EPAR recommends this provider in the wizard by capability, not by an operating-system or Docker-brand allowlist: a local Docker-compatible context such as Docker Desktop or OrbStack must work, `sbx diagnose --output json` must report at least one passing check and no failed checks, and the controller architecture must have a matching native guest template. The `sbx` daemon and each sandbox-private Docker daemon remain separate from that selected host context. After configuration is saved, ordinary startup additionally requires storage, template, and configured architecture-capability admission before any runner starts. The wizard selects `native-only` on every host: it verifies the native guest and private Docker architecture without attempting bundled handlers. Native lifecycle support does not certify foreign workloads.

EPAR preserves the Docker Sandboxes v0.39 missing-sandbox cleanup signatures and recognizes the standardized v0.42.1 response `sandbox '<name>' not found` only when `<name>` exactly matches the instance being stopped or deleted. The live v0.42.1 `ERROR:` prefix and exact `(run 'sbx ls' to see your sandboxes)` suffix are also recognized; unrelated prefixes, suffixes, and names are not. Full v0.42.1 lifecycle compatibility is not live-validated yet; after upgrading, validate it in a disposable pool and treat any other new provider behavior as unsupported until verified.
...
The initial wizard offers `EPAR verified prebuilt Full` as the default, followed by verified prebuilt Act and explicit local-build Full/Act choices. Full uses `ghcr.io/solutionforest/ephemeral-action-runner/docker-sandboxes-template:full-latest`; Act uses the matching `:act-latest` alias. Both paths verify an immutable digest plus GitHub/Sigstore attestation in-process before activation and materialize the first-use Sandbox runtime locally. Host and enterprise CA roots are overlaid where safe; custom scripts are intentionally empty in the base path and create only a small local derivative when configured. Prebuilt acquisition size is resolved during startup rather than presented as the local Buildx estimate. There is no silent fallback to a local build or another provider. See [Docker Sandboxes prebuilt image publication](../development/docker-sandboxes-prebuilt.md) for the public recipe, catalog, attestation, promotion, and rollback contracts.
...
The reusable template disables and masks Ubuntu's periodic apt units. Docker Sandboxes also launches one fixed, best-effort `apt-get update -qq -y` during guest boot; before GitHub runner registration EPAR recognizes the exact parent command forms used by supported legacy releases and v0.43.0, verifies the shell executable and complete argument vector, verifies the child `apt-get` executable and complete argument vector, and terminates only that bootstrap refresh. It then waits for every apt/dpkg process to release its locks. An unexpected package operation is never killed by a broad name or pattern and instead fails that candidate closed after the bounded wait, first emitting the non-secret retry marker `EPAR_CANDIDATE_RECOVERY=package-manager-contention` for controller diagnostics. The registered, replacement-enabled supervisor quarantines the candidate, preserves its capacity slot through exact reconciliation, and retries allocation with the candidate-recovery backoff; matching diagnostic prose without the marker remains terminal. This keeps warm replacement runners from inheriting `/var/lib/apt/lists/lock`, while ordinary workflow-initiated package commands remain available afterward.
...
sbx v0.42.1 injects `SSH_AUTH_SOCK=/run/ssh-agent.sock` even with persistent forwarding disabled, and a create-time empty environment override does not remove it. EPAR normalizes only this exact default when `/run` is traversable, no endpoint or symlink exists, and no gateway or agent PID is supplied. Startup and runtime verification still reject real sockets, files, directories, dangling symlinks, non-default agent paths, gateways, and agent PIDs. This compatibility adjustment requires an updated template as well as the controller: an existing published prebuilt image retains its old embedded checks until a fixed package is published and selected. Changing checkout scripts alone does not modify a verified prebuilt artifact.

1. Run `./start` with no config and select Docker Sandboxes when its tooling and diagnostics pass. Choose verified prebuilt Full (default), verified prebuilt Act, local Catthehacker Full, or local Catthehacker Act, then review the non-blocking physical-growth estimate or the startup-resolved prebuilt acquisition note. The local Full path warns that preparation can take 30–60 minutes. The generated config uses no custom install scripts; add them afterward when a small local derivative is intended. The wizard's initial screen keeps compatibility providers behind `C. Show compatibility providers`; their specialized and custom tags do not change Docker Sandboxes' requirement for a private Docker daemon and runtime closure.
...
Docker Sandboxes can automatically forward the host SSH agent when its shared daemon inherits `SSH_AUTH_SOCK`. That would expose a host credential capability to every sandbox created by that daemon, so EPAR rejects any guest containing `SSH_AUTH_SOCK`, `SSH_AUTH_SOCK_GATEWAY`, `SSH_AGENT_PID`, or `/run/ssh-agent.sock`. EPAR removes these variables whenever it launches Docker Sandboxes commands, so a stopped daemon that those commands auto-start is sanitized. If creation reports the known `failed to run sandbox container` signature, EPAR preserves that original error and adds this SSH-daemon remediation hint; unrelated create failures do not receive it. A provider-owned create deadline or provider-command admission timeout is also treated as an ambiguous admission incident: EPAR makes one bounded read-only identity readback, retains a durable capacity fence when identity is unavailable, and may perform the same stop-wait-start recovery. During automatic recovery, one host-wide provider lease spans the complete authoritative census, durable reservation, daemon intervention, and three stable inventory probes; cooperating EPAR controllers therefore cannot mutate the shared daemon or create a sandbox in the gap. The host-scoped recovery ledger preserves its cooldown, one-intervention admission latch, and bounded exact identity census across EPAR controller restarts and fails closed if its checksum or schema cannot be verified. The installed `sbx ls --json` API is treated as the complete host-wide snapshot because it exposes no independent count or generation; EPAR rejects duplicate keys and durable-identity omissions, but cannot prove that a syntactically valid response includes an unrelated sandbox owned by a tool that bypasses the lease. In the default `recoveryMode: exclusive-auto`, the pool may then retry lifecycle reconciliation; bounded inventory control-plane failures are recovered with the existing probe and backoff policy. `observe` never probes or mutates the daemon. The installed `sbx` CLI does not expose a durable create-operation identity, so EPAR does not adopt an identityless timed-out create after restart using a matching name or workspace; it remains report-only and capacity-fenced. Coordinate with every process using Docker Sandboxes on the host before any interruption; if the bounded automatic attempt has already been used or manual recovery is required, stop the daemon and restart it from a sanitized environment before retrying EPAR:
...
For sbx v0.42 and later, sanitizing the process environment is not a substitute for checking the persistent `ssh.agentForwardingEnabled` setting. Inspect it with `sbx settings get ssh.agentForwardingEnabled`; when it is enabled, coordinate with other local sbx users before running `sbx settings set ssh.agentForwardingEnabled false` and restarting the daemon using the sequence above. This is a host-wide setting and requires a daemon restart to apply. EPAR does not silently change it. The generic `failed to run sandbox container` message does not by itself prove an SSH-forwarding cause: if creation still fails, preserve the daemon logs and investigate the template entrypoint and runtime instead of repeating restarts. See Docker's [SSH forwarding troubleshooting](https://docs.docker.com/ai/sandboxes/troubleshooting/#sandbox-commits-arent-signed).

Do not relax the verification or merely delete the relay socket: the gateway setting is itself a forwarding capability. A stopped daemon may be started explicitly from the sanitized environment above or auto-started by an EPAR-launched `sbx` command, but that does not override its persistent settings. EPAR limits automatic admission recovery to one daemon restart per incident, including controller reconciliation retries, and never invokes `sbx reset`, `sbx logout`, or wildcard deletion for this condition. Provisioning failures include their redacted original cause in the default console message, even if a later recovery check fails for a different reason.

Historical report-only discoveries are not proof that a sandbox still exists. When a fresh pre-intervention census omits a discovery, EPAR may exclude it from that new census only after an independent, bounded exact-name absence readback under the same host-wide recovery lease. It does not delete the discovery record or infer absence from an old tombstone, an empty inventory, a failed readback, or a changed identity. After a controller crash during recovery, verification-only takeover re-establishes the discovery-only absence proofs before merging expectations; an unknown or newly observed discovery cannot silently reuse an earlier exclusion. Failed proof does not insert a historical phantom into the frozen census. Active lifecycle expectations and the persisted census of an interrupted recovery remain mandatory; this reconciliation does not relax the three stable post-recovery probes or authorize a second intervention. A provider without the independent readback capability retains the existing conservative fence.
...
Local builds run the digest-locked SBOM scanner against the complete runner-template filesystem and export its in-toto/SPDX statement as an ordinary file. This avoids BuildKit's smaller native attestation-export size limit without excluding packages, files, or relationships. Max-mode BuildKit provenance remains enabled. EPAR preserves the scanner statement bytes, validates them before activation, and records their hash alongside the verified template in its metadata; it does not manufacture a scanner subject or claim that this local evidence is independently signed. Evidence is limited to 1 GiB, JSON tokens to 16 MiB, and nesting to 256 levels. Malformed, redirected, ambiguous critical-field, or over-limit evidence fails explicitly rather than being truncated or omitted.
