...
Use `configs/tart.example.yml` for the runner-only image or
`configs/tart.web-e2e.example.yml` for the opt-in web/E2E install script.
Tart on Apple Silicon is ARM64, so workflows that require amd64-only Docker
images should avoid the ARM64 label or handle cross-architecture execution in
the workflow.

When Docker/browser support is selected on ARM64, EPAR exposes a
Chromium-compatible browser through `epar-browser`, `chromium`, and
`chromium-browser`; it is not guaranteed to be Google Chrome.
