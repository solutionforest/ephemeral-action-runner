...
Docker Sandboxes places the listener, guest filesystem, and private Docker daemon inside a dedicated microVM sandbox. It provides EPAR's strongest current host boundary and is the primary provider on Linux, macOS, and Windows when the supported-platform, Docker, and machine-readable `sbx` readiness checks pass. Startup then performs the remaining storage, template, policy-rule, native architecture, runtime, and registration admission checks and fails closed. QEMU is a workload compatibility capability rather than part of the isolation boundary and is disabled by default: `native-only` skips it, `best-effort` warns and continues when sandbox-local `binfmt_misc` is unavailable, and `required` makes it an admission requirement.

Docker Sandboxes may forward a host SSH agent when its shared daemon inherits `SSH_AUTH_SOCK`. EPAR strips SSH-agent variables from child commands, which sanitizes a stopped daemon auto-started through those commands, and rejects any sandbox exposing the socket, gateway, or agent PID. A `failed to run sandbox container` create error receives a conditional remediation hint that preserves the original error and explains how to coordinate a sanitized daemon restart. In the default `exclusive-auto` mode, EPAR may make one bounded stop-wait-start recovery attempt for that exact create-stage signature, a provider-owned create deadline, or a post-create provider-command admission deadline, using one host-wide provider lease from the authoritative inventory census through durable reservation, daemon intervention, and stable post-recovery probes; it also recovers bounded inventory control-plane failures. The provider contract treats one `sbx ls --json` response as the complete host-wide snapshot because the installed API exposes no independent count or generation; EPAR rejects unavailable, malformed, duplicate-key, colliding, or durable-identity-omitting responses, but cannot independently detect a syntactically valid response that silently omits an unrelated non-cooperating sandbox. Cooperating EPAR configurations use the same host lease, and operators must coordinate with tools that bypass it. EPAR stores only the bounded immutable name-to-provider-ID census in the checksummed host-scoped recovery ledger, requires every durable identity in every stable post-recovery probe, and retains interrupted or failed recovery for verification-only takeover until three stable probes succeed. `observe` never probes or mutates the daemon. Operators must not weaken the admission check or use `sbx reset`/`sbx logout` as recovery.

For sbx v0.42+, process-environment sanitization does not override the persistent `ssh.agentForwardingEnabled` setting. Inspect it with `sbx settings get ssh.agentForwardingEnabled`; if enabled, coordinate with other local sbx users before deliberately disabling it with `sbx settings set ssh.agentForwardingEnabled false` and restarting the daemon. This host-wide setting is never changed automatically by EPAR. The generic `failed to run sandbox container` error does not establish an SSH-forwarding cause by itself; retain the isolation checks and inspect runtime and template evidence if remediation does not resolve it. Historical discovery omissions can be excluded only after independent exact absence verification under the recovery lease; verification-only takeover re-establishes those exclusions before merging expectations and never removes identities from the frozen positive census. See [troubleshooting](troubleshooting.md#docker-sandboxes-rejects-a-staging-workspace-because-ssh-agent-forwarding-is-present) and [provider guidance](providers/docker-sandboxes.md).

sbx v0.42.1 supplies an inert `SSH_AUTH_SOCK=/run/ssh-agent.sock` even when forwarding is disabled. EPAR clears only that exact default when `/run` is traversable and the endpoint, any symlink, gateway, and agent PID are all absent. All actual endpoint types and non-default agent paths remain rejected at template startup and runtime verification. The published prebuilt template must also carry this compatibility fix; local source edits do not silently alter a verified package.
