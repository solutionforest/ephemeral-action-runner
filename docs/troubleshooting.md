...
With the default `dockerSandboxes.recoveryMode: exclusive-auto`, EPAR keeps the controller alive and holds one host-wide provider recovery lease from the complete authoritative census through the durable reservation, cold stop, authoritative stopped-state confirmation, configured quiescence interval, sanitized detached start, and three identical parsed safety-evidence inventory snapshots covering name, stable provider ID, status, agent/source, and exact workspaces. During that sequence it lets already-running provider commands drain and gates new provider commands through the same host lock, so another cooperating configuration cannot create a sandbox or race the daemon restart. The installed `sbx ls --json` API is treated as the complete snapshot because it exposes no independent count or generation; duplicate keys and durable-identity omissions fail closed, while an otherwise valid response that omits an unrelated sandbox outside the cooperating lease cannot be independently detected. It backs off after failed attempts and preserves exact capacity while recovery is unresolved. The recovery path never invokes `sbx reset`, `sbx logout`, `sbx prune`, or wildcard deletion. Set `recoveryMode: observe` only when an operator deliberately needs to prevent daemon mutation during maintenance.
...
sbx v0.42.1 also injects the default `SSH_AUTH_SOCK=/run/ssh-agent.sock` when forwarding is disabled and the socket is absent. Older EPAR templates reject that inert value too; repeated daemon restarts or `--env SSH_AUTH_SOCK=` do not fix it. Updated EPAR templates and runtime verification normalize only this exact absent endpoint, with no gateway or PID, while retaining actual forwarding rejection. A published prebuilt template must itself contain the fix; updating only the checkout/controller is insufficient. Do not delete sockets or bypass template verification to work around an old artifact.
...
Docker Sandboxes may forward the host SSH agent through inherited environment or, in sbx v0.42+, persistent forwarding settings. EPAR rejects a sandbox exposing the socket or gateway because a workflow could use host SSH credentials. The generic `failed to run sandbox container` message alone does not prove that forwarding caused the failure; inspect the daemon logs and template entrypoint if the cause remains unclear. An explicit forwarding rejection is not evidence that the staging mount is missing or read-only, and deleting only `/run/ssh-agent.sock` is insufficient when forwarding remains enabled.

For sbx v0.42+, first inspect `sbx settings get ssh.agentForwardingEnabled`. If it is enabled, coordinate with every other local sbx user before running `sbx settings set ssh.agentForwardingEnabled false` and restarting the daemon using the sequence below. The setting is host-wide and requires a restart to apply; sanitizing process environment alone does not override it. EPAR does not change shared settings automatically. See [Docker Sandboxes provider guidance](providers/docker-sandboxes.md) for the isolation requirements. If disabling forwarding and restarting does not fix creation, investigate the original error rather than repeating daemon restarts.

In the default `recoveryMode: exclusive-auto`, EPAR treats a provider-owned create deadline or the known immediate create-stage signature as a bounded admission incident and may perform one stop-wait-start recovery using the existing host-global gates, then retry lifecycle reconciliation. A timed-out create gets one bounded read-only identity readback; if the immutable identity is still unavailable, EPAR retains a durable quarantine and capacity fence instead of treating an empty inventory as proof of absence. The host-scoped recovery ledger persists the admission latch and crash-safety cooldown across EPAR controller restarts; if that ledger is corrupt or unavailable, EPAR fails closed and does not intervene in the daemon. The recovery is limited to one daemon restart per incident, including controller reconciliation retries; `recoveryMode: observe` never probes or mutates the daemon. Coordinate the interruption with every process using the shared Docker Sandboxes daemon. If the automatic attempt has already been used or manual recovery is required, stop it and restart it with all forwarding variables removed before retrying EPAR:
...
Health monitoring shares time with host-trust maintenance. With host trust enabled, each monitoring pass has a bounded budget so health checks cannot indefinitely delay lease refresh. A monitoring-budget deadline is different from a provider command exceeding its own deadline: neither proves a stopped listener, and a controller-owned deadline must not trigger a shared-daemon restart. Increasing `timeouts.commandSeconds` does not extend the shorter monitoring budget. EPAR resumes health work fairly rather than checking further runners with an already-expired context.

Timing estimates for instance checks belong to each exact runner, rise immediately when checks slow down, and decay gradually after faster successful checks. An idle runner therefore cannot overwrite a busy runner's timing estimate. These estimates only schedule work; they do not extend the monitoring budget or suppress genuine failures. Initial and replacement provisioning keep survivor leases maintained independently, preserving one-time busy-job handoffs and fail-closed expiry. Ordinary Docker Sandboxes commands can overlap, but an exclusive recovery admission blocks new commands until existing commands drain. A timeout containing `command not started` was spent waiting for that admission rather than executing a guest or inventory command. Debug logs distinguish command admission wait from execution duration; failed lease writes label any subsequent revocation failure separately.

A template inventory `parse failure` means EPAR could not establish the cache contents, not that the template is missing or corrupt. EPAR retries the read once and, if it still fails, reports a fixed parsing category and stdout byte count without printing the provider payload. It retains uncertain catalog resources rather than treating the result as an empty cache. If the warning persists, capture bounded `sbx template ls --json` output for local inspection; do not delete templates based on this warning alone.

The first unknown-health warning is immediate; repeated warnings for the same exact runner are limited to one every five minutes. The summaries retain the latest failure, failure count, episode duration, check stage, deadline category, and time since the last successful health verification. A return to a known health state ends an announced episode with a recovery message. The warning cooldown survives intermittent recovery so a flapping check cannot restart the flood. A quiet console between summaries does not mean the check succeeded; `logging.level: debug` includes phase-level results and durations when investigating progress. Do not disable host-trust protection or suppress all warnings to work around health-check timeouts; persistent failures still need investigation.
