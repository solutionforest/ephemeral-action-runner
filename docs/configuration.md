...
| `recoveryMode` | `exclusive-auto` or `observe`; `exclusive-auto` | Docker Sandboxes. | `exclusive-auto` is the default and permits EPAR to cold-stop and restart the shared sbx daemon after a bounded inventory control-plane failure. `observe` leaves that failure unhandled, does not mutate the daemon, and follows existing fail-closed startup or reconciliation behavior; use it only as an explicit maintenance opt-out. |
