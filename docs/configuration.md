...
For Docker Sandboxes, the initial wizard offers four ordered choices: `1. EPAR verified prebuilt Full` (default), `2. EPAR verified prebuilt Act`, `3. Catthehacker Ubuntu Full` built locally, and `4. Catthehacker Ubuntu Act` built locally. The official prebuilt references are restricted to `ghcr.io/solutionforest/ephemeral-action-runner/docker-sandboxes-template:full-latest` and `:act-latest`; `image.prebuiltDigest` is an optional lowercase SHA-256 pin, and arbitrary GHCR packages are rejected. The Full prebuilt is closest to the GitHub-hosted runner image and avoids the local 30–60 minute Full build; Act is the smaller critical-dependency profile. The published package carries its catalog-pinned runner identity, so prebuilt mode accepts only `runnerVersion: latest` or an empty value rather than an arbitrary exact version. Startup verifies the immutable digest and GitHub/Sigstore attestation in-process before first-use local Docker Sandboxes materialization; host and enterprise CAs are overlaid where safe, while custom scripts remain a small local derivative. Prebuilt resolution never silently falls back to a local build or another provider.
...
| `replacementRetryInitialSeconds` | integer; `15` | All providers. Greater than `0`. | Initial allocation retry delay for candidate recovery and transient replacement failures. |
| `replacementRetryMaxSeconds` | integer; `1800` | All providers. At least the initial delay. | Upper allocation retry-delay cap for candidate recovery and transient replacement failures. |
| `replacementRetryMultiplier` | number; `2` | All providers. At least `1`. | Backoff multiplier for candidate recovery and transient replacement failures. |
| `replacementRetryJitterPercent` | integer; `20` | All providers. `0` through `100`. | Randomizes candidate-recovery and transient replacement delays to avoid synchronized retries. |

Without `--external-outage-retry`, GitHub `429` and `5xx` responses and transient network failures back off steady-state replacement allocation, while other initial-startup failures remain fail-fast after compensating rollback. Independently, a registered candidate enters recovery during initial capacity and steady-state replacement when its listener ends before online readiness is observed, readiness remains uncertain through its deadline, or the Docker Sandboxes guest helper emits the exact package-manager-contention marker after its bounded wait. The controller retains the candidate's quarantine and capacity slot until exact reconciliation adopts it or safely removes it, pauses only allocation with the configured backoff, and continues reconciliation, cleanup, peer monitoring, and host-trust lease maintenance. Candidate recovery retries indefinitely at the capped delay while the replacement-enabled supervisor runs and resets after successful provisioning or verified healthy adoption. With the external-outage flag enabled, the same delay settings also supervise typed transient external failures across startup and replacement; a longer `Retry-After` or rate-limit reset wins in either path. One-shot verification and explicitly disabled replacement keep their existing behavior. Invalid configuration, authentication, ordinary authorization, trust, admission, durable-state, unmarked guest failures, and unrelated local failures remain terminal.
...
| `recoveryMode` | `exclusive-auto` or `observe`; `exclusive-auto` | Docker Sandboxes. | `exclusive-auto` is the default and permits EPAR one bounded stop-wait-start recovery attempt per provider-owned create-admission incident, including the provider's own create deadline and the known sandbox-container signature; bounded inventory control-plane failures retain their existing probe and backoff retries, but cannot restart the daemon while an admission incident remains open. One host-wide provider lease spans the authoritative inventory census, durable reservation, daemon intervention, and stable verification, so cooperating EPAR configurations cannot create a sandbox in that interval. The host-scoped recovery ledger persists its cooldown, admission-incident latch, and bounded identity census across controller restarts, and a corrupt or unavailable ledger fails closed. The installed `sbx ls --json` API is treated as the complete host-wide snapshot because it exposes no independent count or generation; duplicate keys and durable-identity omissions fail closed, but a syntactically valid response that omits an unrelated non-cooperating sandbox cannot be independently detected. An ambiguous create remains quarantined and consumes capacity until an exact outcome is established. `observe` leaves those failures unhandled, does not probe or mutate the daemon, and follows existing fail-closed startup or reconciliation behavior; use it only as an explicit maintenance opt-out. |
| `recoveryQuiescenceSeconds` | integer from `1` through `300`; `60` | Docker Sandboxes. | Time that the daemon must remain authoritatively stopped before EPAR starts it again. EPAR then requires three identical parsed safety-evidence snapshots across name, stable provider ID, status, agent/source, and exact workspace multiset; record and workspace ordering is ignored, while missing or inconsistent evidence fails closed. |
