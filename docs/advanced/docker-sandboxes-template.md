...
Run `./start` with no configuration. The Docker Sandboxes wizard offers verified EPAR Full and Act packages plus explicit local builds from Catthehacker `full-latest` and `act-latest`, the profiles proven to contain the private Docker daemon and runtime closure required by the template contract. Specialized Catthehacker tags such as `js-latest`, `dotnet-latest`, and `gh-latest` remain valid upstream images for providers that supply their own daemon environment, but Docker Sandboxes rejects them before an expensive build. The wizard verifies a local-build source tag and native platform before showing its size estimate; prebuilt mode instead resolves and verifies the immutable package at startup. The generated config keeps custom install scripts empty until the operator edits it. Normal startup performs acquisition or build plus import, and a provisioning failure leaves the configuration available for a retry without changing distribution.
