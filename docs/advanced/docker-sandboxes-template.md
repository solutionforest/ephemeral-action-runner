...
Docker Sandboxes is current on Linux, macOS, and Windows hosts based on completed cross-platform live build, import, lifecycle, and cleanup testing. Run local admission and workload validation for the exact host and workload; this status does not claim independent certification. Any future certification record should bind the reviewed native-controller source/build, full template identity, cache ID, metadata/archive digests, and reviewed evidence.
