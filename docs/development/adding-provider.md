...
Provider cleanup must target exact identities and record enough immutable evidence for common startup housekeeping to distinguish current, superseded, incomplete, and unknown resources. A provider that can reconstruct an immutable receipt from exact inventory evidence may implement the optional orphan-cleanup capability so the shared manager can recover resources inside the configured pool prefix after a restart; the prefix is only the ownership boundary, never the deletion proof. Never replace an unavailable exact operation with a wildcard, broad prune, reset, or deletion of an unknown/shared resource.
